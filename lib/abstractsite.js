...
AbstractSite.prototype.installRole = function(role, version) {
    ...
    var roleDir = path.join(self.dir, 'roles', role.name());

    // Test if role already exists even exit
    if (exists(path.join(roleDir, version))) {
    ...
    return self.atomic(function () {
        ...
        .then(function(versions) {
            ...
            return mkdir(roleDir, DIR_MODE, true);
        })

        // Clone role repository in roles directory
        .then(function() {
            return Git.Clone(role.path(), path.join(roleDir, version), null);
        })
