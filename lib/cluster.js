...
var DIR_MODE = '755',
    CLUSTER_REGEX = /^(([a-zA-Z]|[a-zA-Z][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z]|[A-Za-z][A-Za-z0-9\-]*[A-Za-z0-9])$/;

module.exports = Cluster;

function Cluster(directory, locker) {
 ...
 * @param {string} name - Cluster name to validate.
 ...
 * @return {string} Cluster's directory.
 */
Cluster.prototype.path = function() {
    return this.dir;
};
 ...
 */
Cluster.prototype.renameHost = function(host, newname) {
    var self     = this,
        previous = host.name();
 ...
 * @return {Promise}
 */
Cluster.prototype.atomic = function(callback) {
    return this._lock.synchronized('cluster:' + this.name(), callback);
};

/**
 * Acquire lock of the server.
 * @return {Promise}
 */
Cluster.prototype.lock = function() {
    return this._lock.acquire('cluster:' + this.name());
};

/**
 * Release lock of the server.
 */
Cluster.prototype.unlock = function() {
